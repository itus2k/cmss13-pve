...
// Itus' To-do list
// you should look up the funny delimb stuff.
// Don't forget to make headless bodies possible, but give them constant damage untill they die, or do a timer till they drop to the ground.
// Define a purple blood type, darker than average.
// Give them a new noise or take it away for good.
// Don't forget to add the variable that transforms them into 'soft stone': Faster movement, lower bullet resistance, dies when beheaded, etc.
